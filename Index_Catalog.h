...
public:
  ...
  Index_Catalog(const Index *indexes, const int noOfIndexes) {
  ...
  operator int() const { return this->noOfIndexes; }
  ...
  Index &operator[](int index) const { return indexes[index]; }

  void operator+=(const Index &idx) { this->add_index(idx); }
  ...
  int add_index(const Index &newIndex) {
    if (index_exists(newIndex.getIndexName()) == true) {
      std::cout << "ERROR: Index with name \"" << newIndex.getIndexName()
  ...
  int drop_index(const std::string &indexName) {
    ...
    if (position != 0) {
      std::cout << "ERROR: Index \"" << indexName << "\" does not exist!"
                << std::endl;
      return -1;
    }
  ...
  int setIndex(const std::string &indexName,
               const std::string &columnName) const {
    for (int i = 0; i < noOfIndexes; i++) {
      if (indexes[i].getIndexName() == indexName) {
        std::cout << "ERROR: Index with name \"" << indexName
                  << "\" already exists!" << std::endl;
        return -1;
      }

      if (indexes[i].getColumnName() == columnName) {
        std::cout << "ERROR: Column \"" << columnName
                  << "\" already has index \"" << indexes[i].getIndexName()
                  << "\"!" << std::endl;
        return -1;
      }
    }
