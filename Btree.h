...
class BTreeNode {
    ...
    std::vector <std::vector<int>> values; // pozitiile unde se afla fiecare key (row index), values[i] corespunde cheii keys[i]
...
public:
    ...
    std::vector<int> search(std::string key) {
        int i = 0;
    ...
    void splitChild(int i, BTreeNode* y) {
        ...
        y->keys.resize(t - 1);
        y->values.resize(t - 1);
        if (!y->isLeaf) {//daca nu e frunza, redimensionam si lista de copii
            y->children.resize(t);
        }
    ...
    void insertNonFull(std::string key, int rowIndex) {
